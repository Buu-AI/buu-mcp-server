...
import { SubthreadGenerateParamsSchema, SubthreadGetAllParamsSchema, SubthreadGetParamsSchema } from "../types/Subthread";
...
export const registerSubthreadTools = (server: FastMCP, client: BuuFunServerClient) => {
  server.addTool({
    ...
    execute: async ({ style, prompt, imageUrl, strength, threadId }) => {
      const response = await client.subthread.generate({ style, prompt, imageUrl, strength, threadId });
  ...
  server.addTool({
    ...
    execute: async ({ pagination, filters }) => {
      // @ts-expect-error OrderDirection does not match
